...
def mask_ego_points(points, mask_range = [-1.95, -1.1, 2.95, 1.1]):
    """
...
def lidar_project_n4ton4_normal(lidar_data, extrinsic):
    """
...
# @profile
def lidar_project_einsum(lidar_data, extrinsic):
    if lidar_data.shape[-1] == 4:
        lidar_int = lidar_data[:, 3]
        lidar_xyz = lidar_data[:, :3]
        dim_4 = True
    else:
        lidar_xyz = lidar_data
        dim_4 = False
    lidar_4 = np.pad(lidar_xyz,((0,0),(0,1)),mode='constant',constant_values=1)
    result = np.einsum("ij, nj->ni", extrinsic, lidar_4)
    if dim_4:
        result[:,-1] = np.expand_dims(lidar_int, -1)
    else:
        result = result[:,:3]
    return result
