...
class V2X_fusion(nn.Module):
    ...
    def __init__(
        ...
        # 文章输入通道为64
        # 特征大小为128，128也就是BEV划分大小
        # grid_conf用于特征转换器
        # start_out_channels=64 extra_in_channels=0
        # n_spatial_layers_between_temporal_layers=2，在temporal层之间的空间层数为2
        # use_pyramid_pooling=True，使用空间金字塔池化
        # input_relative_pose=True，使用相对位姿输入
        # with_skip_connect=True，使用跳跃连接
        self.num_agents = num_agents # 代理的数目，全配置为5
        self.compression_conv = compression_conv # 是否使用压缩卷积
        self.input_relative_pose = input_relative_pose # 输入相对位姿
        self.warper = FeatureWarper(grid_conf=grid_conf) # 特征转换器

        h, w = input_shape # 输入特征的高和宽
        modules = []

        block_in_channels = in_channels
        block_out_channels = start_out_channels

        if self.input_relative_pose: # 如果输入相对位姿则需要添加6自由度位姿信息
            # using 6DoF ego_pose as extra features for input
            block_in_channels += 6

        n_agents_layers = num_agents - 1 # 全配置输入为 5 - 4 = 1
        # n_agents_layers = 1

        for _ in range(n_agents_layers): # 对于其他的agent需要使用视角变换
    ...
    def forward(self, x, relative_pose, aug_transform=None):
        """
        V2X fusion module. We choose the five-agent V2X setting, 
        and compare the performance of utilizing average pooling 
        baseline and different state-of-the-art V2X fusion modules 
        as V2XFormer's fusion module. The V2X fusion methods used 
        include DiscoNet (Li et al. 2021), V2X-ViT (Xu et al. 2022b), 
        and CoBEVT (Xu et al. 2022a). As shown in Table 3, CoBEVT 
        achieves the best performance in all three tasks and we will 
        use this V2X fusion module for the following experiments.

        V2X融合模块。我们选择了五代理V2X设置，并比较了使用平均池基线和不同最先进的
        V2X融合模块作为V2XFormer的融合模块的性能。使用的V2X融合方法包括DiscoNet
          (Li et al. 2021)、V2X- vit (Xu et al. 2022b)和CoBEVT (Xu et al. 2022a)。
          如表3所示，CoBEVT在所有三个任务中都达到了最佳性能，我们将在接下来的实验中使用该V2X融合模块。
        """
        ...
        x = self.warper.cumulative_warp_features_v2x(x, relative_pose, mode='bilinear',
                                                            bev_transform=aug_transform)
        # 上边这个是啥？？？输入[1, 5, 64, 128, 128]，以及相对位姿，
        # 这里应该是第二阶段融合之前，利用位姿硬投影然后转换BEV特征
...
# ===== 测试利用直接平均的方式融合特征 ===== #
        ...
        # # if want just avg bev feature 这里应该在测试平均基线
        # ego_feature = x[:, 0].clone() # ego自己的特征先取出来
        # output_feature = x.mean(axis=1) # 多代理的BEV直接融合
        # # 更换通道？？？？？？
        # # big impact
