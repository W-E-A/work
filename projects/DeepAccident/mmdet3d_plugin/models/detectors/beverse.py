...
class BEVerse_V2X(BEVerse):
    ...
    def extract_img_feat(self, img, img_metas, future_egomotion=None,
        ...
        imgs = torch.cat([img_single[0] for img_single in img], dim=0) # 输入的batch，一个元素是img+xx的东西
        ...
        B, S, N, C, imH, imW = imgs.shape # B：S: N: C: 图像通道 H：图像高度 W：图像宽度 ？？？

        imgs = imgs.view(B * S * N, C, imH, imW) # 变为NCHW的形式
        x = self.img_backbone(imgs) # 利用swin-transformer来提取图像的特征

        if self.with_img_neck:
            x = self.img_neck(x) # 如果有img的neck层，文章的配置是fpn-lss，针对lss的fpn
        ...
        # lifting with LSS # 文章利用lss进行lift
        x = self.transformer([x] + [rots, trans, intrins, post_rots, post_trans])

        torch.cuda.synchronize() # 为什么要同步？应该是同步每个时间步的数据？
        t_BEV = time.time()
        ...
        # temporal processing # 这里是spatial-temporal BEV Encoder么
        ...
        torch.cuda.synchronize() # ？？？同步每个agent的bev特征提取？
        t_temporal = time.time()


        relative_pose = relative_pose_to_ego[0]

        # pdb.set_trace() # 融合所有代理的BEV特征
    ...
    def forward_train(self,
        ...
        # 输入图像信息，提取图像的特征，这里包括了融合以及之前的所有步骤，即这里的输出就是融合特征

        img_feats = self.extract_img_feat(
            img=img_inputs, # 输入图像
            img_metas=img_metas, # 输入图像的元信息
            future_egomotion=future_egomotions,
            aug_transform=aug_transform,
            img_is_valid=img_is_valid,
