...
class ResNetModified(nn.Module):
    ...
    def __init__(
        ...
        assert len(layers) == len(layer_strides) == len(num_filters)
        ...
        self.layers = nn.ModuleList([
            self._make_layer(block, num_filters[idx], layers[idx], stride=layer_strides[idx], 
        ...
        if zero_init_residual and isinstance(block, Bottleneck):
            for m in self.modules():
...
def resnet_modified_basic(layers: list, layer_strides: list, num_filters: list,) -> nn.Module:

    return ResNetModified(BasicBlock, layers, layer_strides, num_filters)
