...
from .simple_plot3d import Canvas_3D, Canvas_BEV, Canvas_BEV_heading_right
...
def visualize(pred_box_tensor, gt_tensor, pcd, pc_range, save_path, method='3d', vis_gt_box=True, vis_pred_box=True, left_hand=False, uncertainty=None):
    ...
    if method == 'bev':
        ...
        canvas_xy, valid_mask = canvas.get_canvas_coords(pcd) # Get Canvas Coords
        canvas.draw_canvas_points(canvas_xy[valid_mask])
        # color_list = [(0, 206, 209),(255, 215,0)]
        # for i, pcd_t in enumerate(pcd[1:2]):
        #     canvas_xy, valid_mask = canvas.get_canvas_coords(pcd_t) # Get Canvas Coords
        ...
        canvas = Canvas_3D(left_hand=left_hand)
        canvas_xy, valid_mask = canvas.get_canvas_coords(pcd)
        canvas.draw_canvas_points(canvas_xy[valid_mask])
