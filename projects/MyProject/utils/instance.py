...
def convert_instance_mask_to_center_and_offset_label(
    instance_img, # [ftu_len, H, W]
    future_rela_motion_matrix, # [ftu_len, 4, 4]
    num_instances, # total obj in the current scene
    ignore_index=255, # do not modify
    sigma=3,
    spatial_extent=None):

    seq_len, h, w = instance_img.shape
    # heatmap
    center_label = np.zeros((seq_len, 1, h, w))# 每一帧都有效
    # offset from parts to centers
    offset_label = ignore_index * np.ones((seq_len, 2, h, w))# 每一帧都有效
    # future flow
    ...
    warped_instance_seg = {}
    for t in range(1, seq_len): # 长度为seq_len - 1
        # 将 t 时刻的 instance_img, 反变换回 t - 1时刻
        warped_inst_t = warp_features(instance_img[t][None, None, ...].astype(np.float32), # 1 1 h w
                                      future_rela_motion_matrix[t][None, ...], # 1 4 4
                                      spatial_extent=spatial_extent)
        warped_instance_seg[t] = warped_inst_t[0, 0] # h w
    ...
    for instance_id in range(1, num_instances + 1):
        ...
        for t in range(seq_len):
            ...
            xc = x[instance_mask].mean().round().astype(np.int32) # 均值中心点x
            yc = y[instance_mask].mean().round().astype(np.int32) # 均值中心点y

            off_x = xc - x
            off_y = yc - y
            g = np.exp(-(off_x ** 2 + off_y ** 2) / sigma ** 2) # 形成高斯分布
            center_label[t, 0] = np.maximum(center_label[t, 0], g) # 高斯点填充到中心图中
            offset_label[t, 0, instance_mask] = off_x[instance_mask] # 有实例的地方填写中心点对于像素的偏差x
            offset_label[t, 1, instance_mask] = off_y[instance_mask] # 有实例的地方填写中心点对于像素的偏差y

            if prev_xc is not None:
                # 在1帧开始

                warped_instance_mask = warped_instance_seg[t] == instance_id
                if warped_instance_mask.sum() > 0: # 当前帧相对于前一帧的flow也就是偏移
                    warped_xc = x[warped_instance_mask].mean().round() # 当前帧在上一帧的中心x
                    warped_yc = y[warped_instance_mask].mean().round() # 当前帧在上一帧的中心y

                    delta_x = warped_xc - prev_xc # 当前中心点对于上一帧的所有点的偏移x
    ...
    for b in range(batch_size):
        ...
        for t in range(seq_len):
            pred_instance_t, _ = get_instance_segmentation_and_centers(
                output['instance_center'][b, t].detach(), # 1, H, W
                output['instance_offset'][b, t].detach(), # 2, H, W
                foreground_masks[b, t].detach()# H, W
            )
            pred_inst_batch.append(pred_instance_t)
        pred_inst.append(torch.stack(pred_inst_batch, dim=0))

    pred_inst = torch.stack(pred_inst).squeeze(2) # B, len, H, W
