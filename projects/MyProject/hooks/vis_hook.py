...
class Det3DVisualizationHookModified(Hook):
    def __init__(self,
                 draw: bool = False,
                 interval: int = 50,
                 score_thr: float = 0.3,
    ...
    def after_val_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        if self.vis_task in [
            ...
            img_path = outputs[0].img_path # type: ignore
        ...
        if self.vis_task in ['lidar_det', 'multi-modality_det', 'lidar_seg']:
            ...
            lidar_path = outputs[0].lidar_path # type: ignore
            num_pts_feats = outputs[0].num_pts_feats # type: ignore
    ...
    def after_test_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        for data_sample in outputs:
            ...
            if self.vis_task in ['mono_det', 'multi-view_det', 'multi-modality_det']:
                ...
                img_path = data_sample.img_path # type: ignore
                if isinstance(img_path, list):
                    img = []
                    for single_img_path in img_path:
                        img_bytes = get(
            ...
            if self.vis_task in ['lidar_det', 'multi-modality_det', 'lidar_seg']:
                ...
                lidar_path = data_sample.lidar_path # type: ignore
                num_pts_feats = data_sample.num_pts_feats # type: ignore
                if lidar_path.endswith('.npz'):
                    points = np.load(lidar_path)['data']
                else:
                    pts_bytes = get(lidar_path, backend_args=self.backend_args)
