...
def DeepAccident_det_eval(gt_annos,
    ...
    """
    # 'car', 'van', 'truck', 'cyclist', 'motorcycle', 'pedestrian'
    # overlap_0_7 = np.array([0.7, 0.7, 0.7, 0.5, 0.5, 0.3])  # only this line for 3d bbox (strict)
    # overlap_0_5 = np.array([0.5, 0.5, 0.5, 0.3, 0.3, 0.15])  # only this line for 3d bbox (loose)
    overlap_0_7 = np.array([0.3, 0.3, 0.3, 0.2, 0.2, 0.2])  # only this line for 3d bbox (strict)
    overlap_0_5 = np.array([0.15, 0.15, 0.15, 0.1, 0.1, 0.1])  # only this line for 3d bbox (loose)
    ...
    if eval_types == 'iou_mAP':
        ...
        for j, curcls in enumerate(current_classes):
            ...
            for i in range(min_overlaps.shape[0]):
                ...
                if mAP3d is not None:
                    easy_mAP = int(mAP3d[j, 0, i] * 100) / 100 # type: ignore
                    moderate_mAP = int(mAP3d[j, 1, i] * 100) / 100 # type: ignore
                    hard_mAP = int(mAP3d[j, 2, i] * 100) / 100 # type: ignore
                ...
                for idx in range(3):
                    ...
                    if mAP3d is not None:
                        ret_dict[f'{prefix}_3D_{postfix}'] = mAP3d[j, idx, i] # type: ignore
        ...
        result += '\nmAP_by_category@\n'
        mAP3d_cls = mAP3d.mean(axis=-1) # type: ignore
        ...
        result += ('\nOverall AP@{}, {}, {}:\n'.format(*difficulty))
        if mAP3d is not None:
            mAP3d_distance = mAP3d.mean(axis=0) # type: ignore
        ...
        for idx in range(3):
            ...
            if mAP3d is not None:
                ret_dict[f'CARLA/Overall_3D_{postfix}'] = mAP3d_distance[idx] # type: ignore

        mAP3d_all = mAP3d.mean(axis=0) # type: ignore
